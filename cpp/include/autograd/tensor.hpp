...
// Up to kInline floats are stored inline (no heap); larger tensors heap-allocate.
// Data is column-major in the last two dimensions to match Eigen::MatrixXf.
class Tensor {
public:
  ...
  float  operator()(uint32_t r, uint32_t c) const noexcept { return raw_data()[r + c * rows()]; }
  float &operator()(uint32_t r, uint32_t c)       noexcept { return raw_data()[r + c * rows()]; }

  // ── Eigen bridge ────────────────────────────────────────────────────────────
  // map() views the first batch slice (or the whole tensor if ndim <= 2).
...
private:
  ...
  const float *raw_data()     const noexcept { return is_inline() ? inline_data_ : heap_data_; }
  void         alloc_heap();
